...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `Writely-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("writely.desktop");
}
...
if (process.platform === "darwin" && app.getName() !== "Writely") {
  app.setName("Writely");
}
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>Writely sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
