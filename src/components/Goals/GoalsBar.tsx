...
export const GoalsBar: React.FC<Props> = ({ goals, onChange }) => {
  const Pill = ({ active, onClick, children }: any) => (
    <button
      onClick={onClick}
      className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-all ${active ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase">Writing Goals</h3>
        <span className="text-[11px] text-slate-500">{domainHint[goals.domain]}</span>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div>
          <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1.5 tracking-wide">Audience</div>
        ...
        <div>
          <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1.5 tracking-wide">Formality</div>
          <div className="flex gap-1.5 flex-wrap">
            {formalities.map(f => (
        ...
        <div>
          <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1.5 tracking-wide">Domain</div>
          <div className="flex gap-1.5 flex-wrap">
            {domains.map(d => (
        ...
        <div>
          <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1.5 tracking-wide">Intent</div>
          <div className="flex gap-1.5 flex-wrap">
            {intents.map(i => (
